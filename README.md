...
| **Day 11-13** | **MHA & Safe Softmax**<br>[`mha_forward.py`]() | **Attention Is All You Need**<br>🔗 [PDF](https://arxiv.org/pdf/1706.03762.pdf) <br>🎯 *精读 Sec 3.2* | 1. 手写因果掩码 (Causal Mask) 的广播机制。<br>2. `safe_softmax` 中使用 `x - max(x)` 技巧防止溢出。 |
| **Day 14-17** | **🔥 MoE Router & Experts**<br>[`moe_layer.py`]() | **Mixtral 8x7B**<br>🔗[PDF](https://arxiv.org/pdf/2401.04088.pdf) <br>🎯 *精读 Sec 2.1* | 1. 编写 Top-K Router 得到权重。<br>2. **[算子核心]** 手撕 `scatter` 把 Token 分发给 Expert，计算完再 `gather` 加权组合。 |
| **Day 18-20** | **SwiGLU & CE Loss**<br>[`swiglu_loss.py`]() | **GLU Variants**<br>🔗 [PDF](https://arxiv.org/pdf/2002.05202.pdf) <br>🎯 *精读 Sec 2 (公式 5/6)* | 1. 用 `.mul_()` 就地操作 SwiGLU 减少显存。<br>2. 用 `LogSumExp` 技巧手撕稳健的 CrossEntropyLoss。 |
...
| **Day 32-35** | **🔥 MLA Attention**<br>[`mla_attention.py`]() | **DeepSeek-V2**<br>🔗 [PDF](https://arxiv.org/pdf/2405.04434.pdf) <br>🎯 *精读 Sec 2.1.2 (公式 16-23)* | **字节/阶跃/深信服必考！** 写出将 $c_t$ (Latent Vector) 投影重构并吸收 (Absorb) 权重的逻辑，验证极小的 KV Cache 占用。 |
...
| **Day 58-60** | **🔥 GRPO Loss**<br>[`grpo_loss.py`]() | **DeepSeekMath** 🔗 [PDF](https://arxiv.org/pdf/2402.03300.pdf) *(公式 13)*<br>**DeepSeek-R1** 🔗 [PDF](https://arxiv.org/pdf/2501.12948.pdf) *(Sec 2.2)* | **RL 推理核心！** 摒弃 Critic 网络：对同 Prompt 采样 $N$ 个回答，计算 Reward 的组内均值和标准差，归一化得到 Advantage (优势函数)。 |
